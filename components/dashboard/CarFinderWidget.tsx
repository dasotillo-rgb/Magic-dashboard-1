...
const CarFinderWidget: React.FC<Props> = ({ borderColor = 'border-blue-500/20' }) => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const res = await fetch('/api/cars?overview=true');
            const data = await res.json();
            if (!data.error) {
                // Only profitable deals, sorted by profit descending
                const profitable = (data.listings || [])
                    .filter((l: CarListing) => l.estimatedProfit >= 2000)
    ...
    useEffect(() => {
        fetchData();
        const iv = setInterval(fetchData, 600000);
    ...
    return (
        <div className={`bg-[#1C1C1E]/60 border ${borderColor} rounded-[2rem] p-5 flex flex-col h-full transition-all hover:bg-white/5 overflow-hidden`}>
            ...
            {/* Subtitle */}
            <p className="text-[9px] text-gray-600 mb-2.5">BMW X3 & 330 · &lt;100k km · Profit ≥ €3.000 ↓</p>
            ...
            <div className="space-y-1.5 flex-1 overflow-y-auto min-h-0 pr-1 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
                {loading && listings.length === 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-6">
                        <p className="text-gray-500 text-[10px] font-mono">Sin oportunidades &gt; €3k profit ahora</p>
                    </div>
                ) : (
                    listings.map((car, i) => {
            ...
            <div className="mt-2.5 pt-2 border-t border-white/5 flex items-center justify-between">
                <span className="text-[8px] text-gray-600 font-mono">{listings.length} oportunidades · profit ↓</span>
                <Link
                    href="/cars"
